...
namespace fcpp {
...
#if defined FCPP_FUNC_SIGNATURE_PREFIX
        template<typename Type>
        static consteval auto GetLongName() noexcept
        {
            std::string_view pretty_function{ FCPP_FUNC_SIGNATURE };
            auto first = pretty_function.find_first_not_of(' ', pretty_function.find_first_of(FCPP_FUNC_SIGNATURE_PREFIX) + 1);
            auto value = pretty_function.substr(first, pretty_function.find_last_of(FCPP_FUNC_SIGNATURE_SUFFIX) - first);
            return value;
        }
    ...
    concept IsFunc = std::invocable<Func, Args...>&&
        requires (Func&& fn) { { fn() } -> std::convertible_to<T>; };
